...
<c:forEach items="${wjsInfor}" var="i">
<div style="display:inline;">
<span>结算管理>查看${i.tname}</span>
</div>
...
<br><br>
<form id="zdform" action="<%=request.getContextPath()%>/servlet/ManageJiaoyiServlet?do=11" method="post" class="form-inline" role="form">
	...
	<tbody>
		<c:forEach items="${wjslist}" var="w">
		<tr>
			<td><input type="checkbox" name="checkAll" value="${w.id}"/></td>
			<td>${w.outtradeNo}</td>
			<td>${w.title}</td>
			<td>${w.author}</td>
			<td>${w.total}</td>
...
<script>
	...
	function exportJieSuan(){
		...
		var zdid = $("#zdid").val();
		if (ids.length>0) {
			location.href="<%=request.getContextPath()%>/servlet/ManageJiaoyiServlet?do=15&zdid="+zdid+"&ids="+ids;
		}else {
			alert("请选则需要导出的数据！")
		}
	}
