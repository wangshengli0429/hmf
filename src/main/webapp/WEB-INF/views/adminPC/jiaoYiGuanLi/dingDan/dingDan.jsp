...
<form id="dingdanId" action="<%=request.getContextPath()%>/servlet/ManageJiaoyiServlet?do=1" method="post" class="form-inline" role="form">
	...
	<div class="form-group">
	    ...
	    <select name="state" id="state" class="form-control" style="width:178px">
	      	...
	      	<option value="3" ${state=="3"?"selected":""}>已点评</option>
	      	<option value="2" ${state=="2"?"selected":""}>待点评</option>
	      	<option value="1" ${state=="1"?"selected":""}>待上传</option>
	      	<option value="5" ${state=="5"?"selected":""}>退款中</option>
	      	<option value="6" ${state=="6"?"selected":""}>退款完成</option>
	    </select>
	    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	    <label class="form-label" for="name">支付方式：</label>
	    <select name="payment" id="payment" class="form-control" style="width:178px;">
	   		<option value="">全部</option>
	      	<option value="支付宝" ${payment=='支付宝'?"selected":""}>支付宝</option>
	      	<option value="微信" ${payment=='微信'?"selected":""}>微信</option>
	      	<option value="评点卡" ${payment=='评点卡'?"selected":""}>评点卡</option>
	    </select>
	    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	    <label class="form-label" for="name">时间：</label>
...
<thead>
	<tr class="active">
		...
		<td>所属作文</td>
		<td>所属老师</td>
		<td style="text-align:center">老师账号</td>
		<td style="text-align:center">支付时间</td>
	</tr>
</thead>
<tbody>
	<c:forEach items="${ddlist}" var="l">
	<tr>
		<td name="one">${l.outtradeNo}</td>
		<td name="two">${l.sname}</td>
		<td name="three">${l.total}</td>
		<td name="four">${l.payment}</td>
		<td name="five">
			<c:if test="${l.state==1}">
				待上传
			</c:if>
		...
		<c:if test="${ empty l.phone}">
			<td style="text-align:center">-</td>
		</c:if>
		<c:if test="${not empty l.phone}">
			<td style="text-align:center">${l.phone}</td>
		</c:if>
		
		<td name="nine" style="text-align:center">${l.gmtpayment}</td>
	</tr>
	</c:forEach>
	</tbody>
</table>
