...
//var palette = ["#ff545f","#0ac976","#1a9cff",];
...
var selected_tool = tools[7];//初始化使用的工具
...
// var frames = []; //array of {delay: N, undos: [<canvas>], redos: [<canvas>],
// canvas: <canvas>}? array of Frames?
...
var $bottom = $(E("div")).addClass("component-area2").appendTo($V);//颜色div
var $left = $(E("div")).addClass("component-area33").prependTo($H);//左边工具栏div
$left.attr("id","leftTool");
var $right = $(E("div")).addClass("component-area").appendTo($H);

/* ↓↓↓ 添加按钮*/
var $tool4 = $(E("div")).addClass("tool_column").prependTo($H);
/*按钮*/
var $tool12 = $(E("div")).addClass("tool13").prependTo($tool4);
$tool12.attr("id","saveButton");
//$tool12.attr("onclick","file_update()");
var tool12_1 = $(E("div")).addClass("tool10_1").prependTo($tool12);
tool12_1.text("保存");
var $tool11 = $(E("div")).addClass("tool13").prependTo($tool4);
$tool11.attr("id","revokeButton");
var tool11_1 = $(E("div")).addClass("tool10_1").prependTo($tool11);
tool11_1.text("撤销");
var $tool10 = $(E("div")).addClass("tool10").prependTo($tool4);
$tool10.attr("id","commentButton");
var tool10_1 = $(E("div")).addClass("tool10_1").prependTo($tool10);
tool10_1.text("批注");
/*颜色*/
var $tool9 = $(E("div")).addClass("tool9").prependTo($tool4);
var $tool99 = $(E("img")).addClass("tool444").prependTo($tool9);
$tool99.attr("src","images/cursors/fill-bucket.png");
/*画笔*/
var $tool44 = $(E("div")).addClass("tool2").prependTo($tool4);
var $tool444 = $(E("img")).addClass("tool444").prependTo($tool44);
$tool444.attr("src","images/cursors/pencil.png");

var $tool1 = $(E("div")).addClass("tool_column").prependTo($H);
/*颜色-蓝*/
var $tool88 = $(E("div")).addClass("tool88").prependTo($tool1);
/*颜色-绿*/
var $tool77 = $(E("div")).addClass("tool77").prependTo($tool1);
/*颜色-红*/
var $tool6 = $(E("div")).addClass("tool6").prependTo($tool1);
/*画笔8px*/
var $tool3 = $(E("div")).addClass("tool2").prependTo($tool1);
var $tool33 = $(E("div")).addClass("tool33").prependTo($tool3);
/*画笔12px*/
var $tool2 = $(E("div")).addClass("tool2").prependTo($tool1);
var $tool22 = $(E("div")).addClass("tool22").prependTo($tool2);



/* ↑↑↑ 添加按钮*/

var $status_area = $(E("div")).addClass("status-area").appendTo($V);//底部div
$status_area.attr("id","bottomBut");
var $status_text = $(E("div")).addClass("status-text").appendTo($status_area);
var $status_position = $(E("div")).addClass("status-coordinates").appendTo($status_area);//坐标div
...
// Note: a second $ToolBox doesn't work because they use the same tool options
// (which could be remedied)
// and also the UI isn't designed for multiple vertical components (or
// horizontal ones)
// If there's to be extra tools, they should probably get a window, with
// different UI
// so it can display names of the tools, and maybe authors and previews (and not
// necessarily icons)
...
$G.on("keydown", function(e){
	...
	// maybe it should only handle the event if document.activeElement is the
	// body or html element?
	// (or $app could have a tabIndex and no focus style and be focused under
	// various conditions,
	...
	// it's kinda fun, but it's a lot of code for something I've virtually never
	// used
	// (and it's not based on any version of mspaint, it was just a fun idea I
	// had)
	var brush_shapes = {
		circle: [
			0, 1, 0,
...
$G.on("cut copy paste", function(e){
	...
	if(
		...
		// Don't prevent cutting/copying/pasting within inputs or textareas, or
	...
	if(e.type === "copy" || e.type === "cut"){
		if(selection && selection.canvas){
			...
			// viewBox="0 0 ${selection.canvas.width}
			// ${selection.canvas.height}" preserveAspectRatio="xMidYMid slice">
			// <image xlink:href="${data_url}" x="0" y="0"
			// height="${selection.canvas.height}"
			// width="${selection.canvas.width}"/>
			// </svg>
			// `;
		...
		$.each(cd.items, function(i, item){
			if(item.type.match(/^text\/(?:x-data-uri|uri-list|plain)|URL$/)){
				item.getAsString(function(text){
					...
					// but definitely generally show a better error than
					// show_resource_load_error_message()
					// "The information on the Clipboard can't be inserted into
					// Paint."
					// also
					// "Downloading picture"
...
$canvas.on("pointerdown", function(e){
	if(e.button === 0){
		roadObj.stsrtPosi = pointer.x + "," + pointer.y;//设置开始位置
	}
	...
	$G.one("pointerup", function(e){
		...
		roadObj.endPosi = pointer.x + "," + pointer.y;//设置结束位置
		//var str1 = JSON.stringify(roadObj);
		//alert("坐标信息-roadObj："+str1);
...
//		alert("坐标信息集合-globalArr："+str);
		...
		/*
		comment = prompt("请输入您的点评:","请输入点评内容");
		if (comment != null){
			...
			if(le==0){
				...
				roadObj.commentConcent = comment;
				var tempObj = {
						stsrtPosi:roadObj.stsrtPosi,
						endPosi:roadObj.endPosi,
						passRoute:roadObj.passRoute,
