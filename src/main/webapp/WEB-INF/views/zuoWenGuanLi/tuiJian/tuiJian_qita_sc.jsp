	...
	<tr>
	    ...
	    <select name="zwGrade" id="zwGrade" class="form-control" style="width:205px;">
	      	...
	      	<c:forEach items="${geadelist}" var="g">
	    		<option value="${g.code}">${g.grade}</option>
	      	</c:forEach>
	    </select>
	    </td>
	</tr>
	<tr>
	    ...
	    <select name="zwStyle" id="zwStyle" class="form-control" style="width:205px;">
	      	...
	      	<c:forEach items="${stylelist}" var="s">
	    		<option value="${s.code}">${s.grade}</option>
	      	</c:forEach>
	    </select>
	    </td>
	</tr>
...
<table class="table table-hover" style="width: 100%;">
	...
	<tr>
		...
		<td name="one" style="text-align: center"><a href="##" onclick="selectSucai(${l.id})">${l.name}</a></td>
...
function quXiao(){
	...
	function addQita(){
		var d = dialog({
			...
			ok: function () {
				...
				location.href="<%=request.getContextPath()%>/servlet/ManageZuowenServlet?do=30&id="+selectvalue;
			},
			cancelValue: '取消',
			cancel: function () {}
		});
	...
	function selectSucai(id){
		$.ajax({
			url:"<%=request.getContextPath()%>/servlet/ManageZuowenServlet?do=28&id="+id,
			type:"post",
			dataType: "json",
