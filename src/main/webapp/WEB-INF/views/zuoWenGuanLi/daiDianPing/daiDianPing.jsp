...
<form id="daidpId" action="<%=request.getContextPath()%>/servlet/ManageZuowenServlet?do=1" method="post" class="form-inline" role="form">
			...
			<td>
				...
				<c:if test="${not empty l.teacherId}">
					...
					<button type="button" class="btn btn-info" style="background:#54a7fd;" data-toggle="button" onclick="updateTeacher(${l.id})" ${l.tea_state == 1?"disabled": ""} >更换老师</button>
					 --%>
					<button type="button" class="btn btn-info" style="background:#54a7fd;" data-toggle="button" onclick="updateTeacher(${l.id})">更换老师</button>
				</c:if>
